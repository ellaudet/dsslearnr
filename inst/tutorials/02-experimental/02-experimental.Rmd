...
The asprin would have <em>decreased</em> your level of headache by two levels (in a scale from 1 to 5). (Notice that it is a decrease because the treatment had a negative effect on the outcome.)<br><br> For information on individual causal effects, review section 2.3 of DSS.",
    random_answer_order = TRUE,
    allow_retry = TRUE
  ),
...
<br> For information on the fundamental problem of causal inference, review section 2.3 of DSS.",
   ...
   question("<span style=color:dodgerblue> Q7:</span> Is it possible for the same treatment to have different effects for different individuals as we computed in the hypothetical example above?",
    ...
    post_message = "Yes, it is possible. There is no guarantee that a treatment has the same effects for all individuals. For example, an aspirin might help reduce the headache of one individual, have severe adverse effects on another, and have no effect whatsoever on yet another.
  ...
  When estimating average causal effects, we distinguish between the treatment group and the control group. If we were interested in estimating the effect of taking an aspirin on the level of intensity of a headache, the treatment group would consiste of...",
  ...
  question("<span style=color:dodgerblue> Q9 - A MULTI-ANSWER QUESTION:</span> In chapter 2, we discuss a way by which when estimating average treatment effects we can obtain good approximations for the counterfactual outcomes (which are, by definition, unobserved). What is that way? (Note: You must select ALL true statements for the answer to be correct.)",
    ...
    post_message = "There are four correct answers here: (1) When we are analyzing experimental data, (2) When we are analyzing data from a randomized experiment, and (3) When treatment is assigned at random (also known as random treatment assignment), and (4) When treatment and control groups were comparable before the administration of the treatment. (The first three answers all mean the same thing.)<br><br>
    For an explanation, review section 2.4 of DSS.",
    random_answer_order = TRUE,
    allow_retry = TRUE
  ),
...
The difference-in-means estimator has a similar formula, but it uses  <em>average</em> outcomes because it aims to estimate the <em>average</em> causal effect of a treatment on an outcome (among other differences):<br><br>
