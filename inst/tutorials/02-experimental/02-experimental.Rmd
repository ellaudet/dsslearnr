...
<ul>
  ...
  <li>Multi-answer questions are clearly labeled</li>
...
<br> For information on the fundamental problem of causal inference, review section 2.3 of DSS.",
   ...
   question("<span style=color:dodgerblue> Q7:</span> Is it possible for the same treatment to have different effects for different individuals?",
  ...
  question("<span style=color:dodgerblue> Q9 - A MULTI-ANSWER QUESTION:</span> In chapter 2, we discuss how we can obtain good approximations for the counterfactual outcomes, which are unobserved by definition. What is that way? (Note: You must select ALL true statements for the answer to be correct.) </span>",
...
<br> <span style=color:dodgerblue> **Q1 - Create the Treatment Variable:** </span> **Let's start by using the function `ifelse()` to create the treatment variable `small`, identifying the students who were randomly assigned to attend a small class. Make sure to store the values as a new variable in the dataframe `star`.**
...
<br> <span style=color:dodgerblue> **Q4 - Review How to Compute and Interpret Means: </span> Now, use the `mean()` function to compute the proportion of students in Project STAR who were assigned to attend a small class?**
...
<br> <span style=color:dodgerblue> **Q12 - Compute the Difference-in-Means Estimator for Graduated:** </span> **Although we could compute the difference-in-means estimator as the difference between the last two means (`0.8735 - 0.8665`), it is best to ask R to compute it all at once, using functions. What is the code to compute the difference-in-means estimator for *graduated*?**
...
question("<span style=color:dodgerblue> Q16: </span> When estimating the average causal effect of attending a small class on the third-grade math test, what is the outcome we are estimating the effect on?",
...
question("<span style=color:dodgerblue> Q18: </span> When estimating the average causal effect of attending a small class on the probability of graduating from high school, what is the outcome we are estimating the effect on?",
...
question("<span style=color:dodgerblue> Q19: </span> When estimating the average causal effect of attending a small class on the probability of graduating from high school, what is the direction, size, and unit of measurement of the average causal effect?",
...
quiz(caption="",
  question("<span style=color:dodgerblue> Q20: </span> What is the estimated average causal effect of attending a small class on the third-grade math test?",
...
question("<span style=color:dodgerblue> Q21: </span> What is the estimated average causal effect of attending a small class on the probability of graduating from high school?",
...
To generate a summary report of your work, enter your name below and click the *Generate Summary Report* button. The summary will appear on the screen. Take a screenshot, if your instructor asks for proof of completion.
...
shiny::observeEvent(
  input$get_score, 
  {
    ...
    output$chapter = shiny::renderText(
      paste("SUMMARY REPORT OF: Review Exercises for", as.character(chapter_id),
