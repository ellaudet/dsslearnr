...
question(paste0("<span style=color:dodgerblue> Q", qnum <- qnum+1, ":</span> The formula for the difference-in-means estimator is..."),
    ...
    answer("outcome for the treatment group - outcome for the control group", message="Hint: each group contains multiple individuals, so they have multiple outcomes."),
...
question(paste0("<span style=color:dodgerblue> Q", qnum <- qnum+1, ":</span> Suppose we want to estimate the average causal effect of taking an aspirin on headache intensity and recruit 600 volunteers. How should we decide who should take an aspirin?"),
    ...
    answer("We should give the aspirin to whomever chooses to take one",
           message = "If individuals choose whether to take an aspirin, the treatment and control groups would likely not be comparable. For example, people with prior health conditions may be less likely to take aspirin and may also have worse headaches. Under this circumstances, we would not know whether differences after treatment were due to the aspirin or to pre-existing differences."),
...
question(paste0("<span style=color:dodgerblue> Q", qnum <- qnum+1, ":</span> What would be the difference-in-means estimator in the randomized experiment from the previous exercise?"),
    ...
    answer("average headache intensity before treatment among those randomly assigned to take the aspirin <br>- average headache intensity before treatment among those randomly assigned NOT to take the aspirin", 
           message = "This does not compare the post-treatment outcomes of the treatment and control groups, and randomization should make this pre-treatment difference approximately zero, on average."),
...
quiz(caption=NULL,
  ...
  question(paste0("<span style=color:dodgerblue> Q", qnum <- qnum+1, ":</span> If Y is binary,  what is the unit of measurement of the difference-in-means estimate?"),
    ...
    answer("\"FALSE\"", message = "TRUE and FALSE are special values in R, so we do not write them in quotes."),
    answer("NO"),
    answer("NOT TRUE"),
    answer("6"),
...
Suppose `course$present` is a character variable coded 'yes' for students who attended class and 'no' for those who did not, and we want to create `course$attended`, coded 1 for students who attended and 0 otherwise. Which of the following pieces of code would work? "),
    ...
    answer("`attended <- ifelse(course$present=='yes', 1, 0)`", message = "This code would store the values as a new object called `attended`, instead of as a new variable inside the object `course`."),
...
quiz(caption=NULL,
    ...
    answer("the students in Project STAR were randomly selected from a population", message= "This matters when we generalize the results to a population other than the sample of observations studied, not for the validity of the estimate within the sample of observations studied."),    
