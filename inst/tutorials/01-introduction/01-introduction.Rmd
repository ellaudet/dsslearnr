...
question("<span style=color:dodgerblue> Q3: </span>How do we run code that has already been written on an R script?",
  ...
  question("<span style=color:dodgerblue> Q9 - A MULTI-ANSWER QUESTION: </span> As we learn in Section 1.6.3 of DSS, a function: <br><br> 
    ...
    answer("The name of the function determines the action(s) the function performs, the input(s) it requires, and the output(s) it produces.", correct = TRUE),
...
```{r topic1_r1, exercise=TRUE}

```

<div id="topic1_r1-hint">
Hint: You may want to use the `+` operator.
</div>

```{r topic1_r1-solution}
30+6
```
...
```{r topic1_r2, exercise = TRUE}

```

<div id="topic1_r2-hint">
...
```{r topic1_r3, exercise=TRUE, exercise.setup = "thirtysix"}

```

<div id="topic1_r3-hint">
Hint: To ask R to show us the contents of an object, we run the name of the object.
</div>

```{r topic1_r3-solution}
thirtysix
```
...
<br> <span style=color:dodgerblue> **Q1:** </span> **Use the `head()` function to ask R to show the first six observations of the object `star`**

```{r star, echo=FALSE}
star <- read.csv("data/STAR.csv")
```

```{r topic2_r1, exercise=TRUE, exercise.setup = "star"}
...
<br> <span style=color:dodgerblue> **Q2:** </span> **Now, use the `head()` function again to ask R to show the first *eight* observations of the object `star`**

```{r topic2_r2, exercise=TRUE, exercise.setup = "star"}
...
```{r topic2_r2_inter}
quiz(caption="",
       question("<span style=color:dodgerblue> Q2 - Interpretation: </span>Looking at the output of `head()` above, we can see that ....",
  ...
  question("<span style=color:dodgerblue> Q2 - Interpretation: </span>Looking at the output of `head()` above, we can substantively interpret the first observation. The first observation represents ....",
...
```{r topic2_r3, exercise=TRUE, exercise.setup = "star"}

```

<div id="topic2_r3-hint">
...
question("<span style=color:dodgerblue> Q4: </span>If the unit of observation is students and *attended* = {0, 0, 1, 1, 1, 0}, what is $\\overline{attended}$ ?",
  ...
  Interpretation: The interpretation of the mean of a variable depends on whether the variable we are computing the mean of is binary or non-binary. The mean of a binary variable should be interpreted as proportion and in percentages, after multiplying the output by 100.<br><br> Here, *attendance* is binary (because it can take only two values: 1s and 0s), so the mean of *attendance* should be interpreted as a proportion after multiplying the output by 100 (0.5*100=50%). HEREEEEE 
  In particular, it should be interpreted as the proportion of the observations that have the characteristic identified by the variable. In this case, the proportion of students who attended class.<br><br>
  ...
  Interpretation: The interpretation of the mean of a variable depends on whether the variable we are computing the mean of is binary or non-binary. The mean of a non-binary variable should be interpreted as an average and in the same unit of measurement as the variable. <br><br>Here, *distance* is non-binary (because it can take more than two values) and measured in miles, so the mean of *distance* should be interpreted as an average and in miles.",
    random_answer_order = TRUE,
    allow_retry = TRUE
  )
)
...
```{r topic3_r1, exercise=TRUE, exercise.setup = "star"}

```

<div id="topic3_r1-hint">
...
quiz(caption="",
    ...
    post_message = "The correct answer is: Among the students who participated in Project STAR, the average score in the third-grade reading test is about 629 points.<br><br>
     Recall: The interpretation of the mean of a variable depends on whether the variable we are computing the mean of is binary or non-binary. The mean of a non-binary variable should be interpreted as an average and in the same unit of measurement as the variable.  <br><br>Here, as we can see in the output of `head()` above, *reading* is non-binary because it can take more than two values. And, as we can read in the description of the variable in the table above, *reading* is measured in points. So, the mean of *reading* should be interpreted as an average and in points.",
    random_answer_order = TRUE,
    allow_retry = TRUE
  ))
```
...
```{r topic3_r2, exercise=TRUE, exercise.setup = "star"}

```

<div id="topic3_r2-hint">
Hint: The `mean()` function calculates the mean of a variable. The only required argument is the code identifying the variable. To identify the variable, we can use the `$` character.<br><br> 
We use the `$` character to access a variable inside a dataframe. To its left, we specify the name of the object where the dataframe is stored (without quotes). To its right, we specify the name of the variable (without quotes). Example: `data$variable`.
</div>

```{r topic3_r2-solution}
mean(star$graduated)
```
...
quiz(caption="",
    ...
    answer("Among the students who participated in Project STAR, the average graduation is 0.87", message="Is *graduated* binary or non-binary? <br><br>The interpretation of the mean of a variable depends on whether the variable we are computing the mean of is binary or non-binary. The mean of a binary variable should be interpreted as proportion and in percentages, after multiplying the output by 100."),
    ...
    Recall: The interpretation of the mean of a variable depends on whether the variable we are computing the mean of is binary or non-binary. The mean of a binary variable should be interpreted as proportion and in percentages, after multiplying the output by 100. <br><br>Here, as we can see in the output of `head()` above, *graduated* is binary because it can take only two values: 1s and 0s. So, the mean of *graduated* should be interpreted as a proportion after multiplying the output by 100 (0.87*100=87%). In particular, it should be interpreted as the proportion of the observations that have the characteristic identified by the variable. In this case, the proportion of students who graduated from high school.",
    random_answer_order = TRUE,
    allow_retry = TRUE
  ))
```
...
```{r topic3_r3, exercise=TRUE, exercise.setup = "star"}
average(math$STAR) # computes the mean of math
```

<div id="topic3_r3-hint">
Hint: The function that calculates the mean of a variable is `mean()`, not `average()`.<br><br>
...
quiz(caption="",
    ...
    post_message = "The correct answer is: Among the students who participated in Project STAR, the average score in the third-grade math test is about 632 points.<br><br>
     Recall: The interpretation of the mean of a variable depends on whether the variable we are computing the mean of is binary or non-binary. The mean of a non-binary variable should be interpreted as an average and in the same unit of measurement as the variable. <br><br>Here, as we can see in the output of `head()` above, *math* is non-binary because it can take more than two values. And, as we can read in the description of the variable in the table above, *math* is measured in points. So, the mean of *math* should be interpreted as an average and in points.",
    random_answer_order = TRUE,
    allow_retry = TRUE
  ))
```

## <span style=color:dodgerblue> COMPLETION REPORT
Congratulations! You have completed the practice exercises in this tutorial!

Now, you can generate a completion report. Just enter your name in the text box below and click on the *Generate Completion Report* button. A summary of your work will appear on the screen. You may want to take a screenshot of this summary whenever an instructor requests evidence of your work.
...
shiny::observeEvent(
  input$get_score, 
  {
    ...
    output$chapter = shiny::renderText(
      paste("Practice Exercises for:", as.character(tut_reptitle),
    ...
    output$num = shiny::renderText(
      paste("Number of Exercises Answered:", total_questions,""))
    
    output$num_corr = shiny::renderText(
      paste("Number of Exercises Answered Correctly:", n_correct,""))
    
    output$score = shiny::renderText(
      paste("Proportion of Exercises Answered Correctly (Out of Those Answered):",
            round(n_correct/total_questions*100,0),"percent"))
    
    output$score_all = shiny::renderText(
      paste("Proportion of Exercises Answered Correctly (Out of All Exercises in Tutorial):", round(n_correct/num_tutorial*100,0),"percent"))
    ...
    output$system = shiny::renderText(
...
shiny::actionButton("get_score", "Generate Completion Report")
shiny::br()
shiny::br()
shiny::br()
...
shiny::textOutput("system")
shiny::br()
shiny::textOutput("num_tutorial")
shiny::textOutput("num")
shiny::textOutput("num_corr")
shiny::br()
shiny::textOutput("score")
shiny::textOutput("score_all")
shiny::br()
```
