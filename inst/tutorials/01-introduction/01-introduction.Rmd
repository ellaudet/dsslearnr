...
question("How do we run code that has already been written on an R script?",
  ...
  question("What would happen if after running the code from the previous question, we ran the following piece of R code: ` today <- Tuesday `",
  ...
  question("<span style=color:dodgerblue> A MULTI-ANSWER QUESTION: </span> As we learn in Section 1.6.3 of DSS, a function: <br><br> 
          <center>
  ...
  question("<span style=color:dodgerblue> A MULTI-ANSWER QUESTION: </span> If we had run all the code above, which of the following pieces of R code could we use to compute the square root of 3? (Note: You must select ALL true statements for the answer to be correct.)",
  ...
  question("What would happen if after running all the code above, we ran the following piece of R code: ` # square root of three is sqrt(three)`",
...
<br> **Ask R to compute 30+6**

```{r r1, exercise=TRUE, exercise.lines = 1}
...
<br> **Ask R to store `30+6` into an object called `thirtysix`**

```{r r2, exercise = TRUE, exercise.lines = 1}
...
```{r r2-check}
grade_result_strict(
  pass_if(~ identical(thirtysix, 36), "<br><br> Explanation: To create an object, we use the assignment operator `<-`. <br><br>
To the left of the assignment operator, we specify the name of the object (without quotes). <br><br>
...
<br> **Ask R to show you the contents of the object `thirtysix`**

```{r thirtysix}
thirtysix <- 30+6
```

```{r r2b, exercise=TRUE, exercise.setup = "thirtysix", exercise.lines = 1}
...
<br> **Use the `sqrt()` function to ask R to compute the square root of 36**

```{r r3, exercise=TRUE, exercise.setup = "thirtysix", exercise.lines=1}
...
quiz(caption="",
  ...
  question("<span style=color:dodgerblue> A MULTI-ANSWER QUESTION: </span> To set the working directory to the DSS folder on our Desktop, we could... (Note: You must select ALL true statements for the answer to be correct.)",
  ...
  question("<span style=color:dodgerblue> A MULTI-ANSWER QUESTION: </span> If we wanted to take a look at the data stored in the object called *grades*, we could run... (Note: You must select ALL true statements for the answer to be correct.)",
  ...
  question("If we ran ` dim(grades) ` and the output was <span style=color:lightslategray> 76   4</span>, we would conclude that...",
...
For the next few questions, we are going to continue to work with the STAR dataset from Chapters 1 and 2 of DSS. As a reminder, below is the description of the variables in this dataset, where the unit of observation is students.
...
<br> **Suppose the dataset has already been stored in an object called `star`.** 

<br> **Use the `head()` function to ask R to show you the first six observations of the object `star`**

```{r star}
star <- read.csv("data/STAR.csv")
```

```{r r4, exercise=TRUE, exercise.setup = "star", exercise.lines = 1}
...
<br> **Now, use the `head()` function again to ask R to show you just the first *two* observations of the object `star`**

```{r r5, exercise=TRUE, exercise.setup = "star", exercise.lines = 1}

```

<div id="r5-hint">
Hint: To change the number of observations displayed by the `head()` function, we can use the optional argument named `n`. 
</div>
...
```{r r5-check}
grade_code("<br><br>To change the number of observations displayed by the `head()` function, we can use the optional argument named `n`.")
```

<!-- ```{r r4-check} -->
<!-- grade_result( -->
<!--   pass_if(~ identical(.result, head(star)), "<br><br> The function `head()` ...") -->
<!-- ) -->
<!-- ``` -->
